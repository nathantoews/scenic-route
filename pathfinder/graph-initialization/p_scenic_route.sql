drop function if exists p_scenic_route(integer);
create function p_scenic_route(integer) RETURNS TABLE(path int, route text[]) as $$
BEGIN
	...
	return query
		...
		results as (
			...
			FROM pgr_ksp('SELECT * FROM b_graph', 2356, 2357, $1, false)
		),
		pkg as (
