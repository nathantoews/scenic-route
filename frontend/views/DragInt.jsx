...
function Rectangle (x, y, w, h, svgCanvas) {
  this.svgCanvas = document.querySelector('.drag');
  ...
  this.h = h;
  this.stroke = 5;
  this.el = document.createElementNS(this.svgNS, 'rect');
  
  this.el.setAttribute('data-index', this.rectangles.length);
  this.el.setAttribute('class', 'edit-rectangle');
  this.rectangles.push(this);

  this.draw();
  this.svgCanvas.appendChild(this.el);
}
...
interact('.edit-rectangle')
  ...
  .rectChecker(function (element) {
    ...
    var rectangle = this.rectangles[element.getAttribute('data-index')];
  ...
  .draggable({
    ...
    onmove: function (event) {
      var rectangle = this.rectangles[event.target.getAttribute('data-index')];
